...
export const updateMenu = async (name: string, date: Date, menu: { en: object; fi: object }): Promise<void> => {
  try {
    ...
    if (restaurant) {
      ...
      await (await getDb()).collection('restaurants').updateOne({ name }, { $push: { menu: { date, ...menu } } });
    }
  } catch (err) {
    if (err.code === 121) {
